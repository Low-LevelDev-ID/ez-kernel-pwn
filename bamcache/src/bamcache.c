...
static struct kmem_cache *bamcache_cachep;
...
static long bamcache_add(int64_t idx)
{
    ...
    bamcache_arr[idx] = kmem_cache_zalloc(bamcache_cachep, GFP_KERNEL);
    if (!bamcache_arr[idx])
    {
...
static int init_bamcache_driver(void)
{
    ...
    bamcache_cachep = kmem_cache_create_usercopy("bamcache_cache",
                                                 CHUNK_SIZE,
                                                 0, SLAB_HWCACHE_ALIGN | SLAB_PANIC | 
                                                 SLAB_ACCOUNT, 0, CHUNK_SIZE, NULL);
    if (!bamcache_cachep)
    {
        printk(KERN_INFO "[bamcache]: Failed to create bamcache cache... \n");
        return -1;
    }
    bamcache_ctr = 1;
...
static void cleanup_bamcache_driver(void)
{
    ...
    kfree(bamcache_arr);
    kmem_cache_destroy(bamcache_cachep);
